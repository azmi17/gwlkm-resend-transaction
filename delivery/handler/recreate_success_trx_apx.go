...
func RecreateSuccessTransactionApx(ctx *gin.Context) {
	...
	if rerr != nil {
		...
		response := helper.ApiResponse("Recreate apex transaction failed", http.StatusUnprocessableEntity, "failed", errorMesage)
	...
	if er != nil {
		...
		resp.ResponseMessage = "Recreate apex transaction succeeded"
	}
